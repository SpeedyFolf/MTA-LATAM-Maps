...
addEventHandler("onPlayerFinish", getRootElement(), function(rank, time)
	...
	if (DATABASE) then
		if getPedOccupiedVehicle(source) then
			recordsQuery = dbQuery(DATABASE, "SELECT * FROM BadlandsTable WHERE vehiclename = ?", getVehicleNameFromModel(currentVehicleModel))
			recordsResults = dbPoll(recordsQuery, -1)		
			
			-- Updating Database
			if (recordsResults and #recordsResults > 0) then
				if recordsResults[1]["score"] > time then 
					dbExec(DATABASE, "UPDATE BadlandsTable SET score = ?, playername = ? WHERE vehiclename = ?", time, getPlayerName(source), getVehicleNameFromModel(currentVehicleModel))
				end
			else
				dbExec(DATABASE, "INSERT INTO BadlandsTable(playername, vehiclename, score) VALUES (?,?,?)", getPlayerName(source), getVehicleNameFromModel(currentVehicleModel), time)
			end
			
			-- Sort first 164 Records 
			sortQuery = dbQuery(DATABASE, "SELECT * FROM BadlandsTable ORDER BY score ASC LIMIT 164")		
...
addEventHandler("getStats", getRootElement(), function()
	if DATABASE then
		local data = {}
		recordsQuery = dbQuery(DATABASE, "SELECT * FROM BadlandsTable ORDER BY score ASC LIMIT 164")
