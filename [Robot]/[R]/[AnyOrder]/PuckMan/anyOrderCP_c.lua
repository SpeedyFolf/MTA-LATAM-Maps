...
local INTRO_CHAT_MSG = "Move around and eat all the dots in any order."
local INTRO_CHAT_COLOR = "#FFFF00"
...
function collectCheckpoints(target)
	...
	for i = checkpoint, target do
		...
		if (#colshapes == 0) then
			outputConsole("[Any Order CPs] SOMETHING WENT HORRIBLY WRONG")
			iprint("[Any Order CPs] SOMETHING WENT HORRIBLY WRONG")
			break
		end
