...
addEventHandler("onPlayerFinish", getRootElement(), function(rank, time)
	...
	if (DATABASE) then
		if getPedOccupiedVehicle(source) then
			...
			sortQuery = dbQuery(DATABASE, "SELECT * FROM FuckedupTownTable ORDER BY score ASC LIMIT 164")		
...
addEventHandler("getStats", getRootElement(), function()
	if DATABASE then
		local data = {}
		recordsQuery = dbQuery(DATABASE, "SELECT * FROM FuckedupTownTable ORDER BY score ASC LIMIT 164")
