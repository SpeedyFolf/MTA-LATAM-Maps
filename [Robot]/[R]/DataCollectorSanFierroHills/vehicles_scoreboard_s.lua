...
addEventHandler("onPlayerFinish", getRootElement(), function(rank, time)
	...
	if (DATABASE) then
		if getPedOccupiedVehicle(source) then
			recordsQuery = dbQuery(DATABASE, "SELECT * FROM SanFierroTable WHERE vehiclename = ?", getVehicleNameFromModel(currentVehicleModel))
			recordsResults = dbPoll(recordsQuery, -1)		
			
			-- Updating Database
			if (recordsResults and #recordsResults > 0) then
				if recordsResults[1]["score"] > time then 
					dbExec(DATABASE, "UPDATE SanFierroTable SET score = ?, playername = ? WHERE vehiclename = ?", time, getPlayerName(source), getVehicleNameFromModel(currentVehicleModel))
				end
			else
