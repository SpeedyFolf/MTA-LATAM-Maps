...
function craneTimerTick(craneID)
    ...
    if (CRANE_STATE[craneID] == "sleeping") then
        if (CRANE_SLEEPTIMER[craneID] <= 0) then
            if (craneID == 1) then
				...
				outputConsole("[FleischBerg Autos] Rotating crane 1 out of crane 2's way @"..getRealTime().timestamp)
    ...
    elseif (CRANE_STATE[craneID] == "assist") then
        if (getElementAttachedTo(vehicle) ~= false) then
            ...
            local desiredZ = VEHICLE_CRANE_DATA[vehicleModel][craneID]
            local rX,rY,rZ = getElementPosition(CRANE_ROPE[craneID])
            ...
            if (hookHeightDeviation > 0.1 and barDirectionDeviation > 0.1) then
				...
				outputConsole("[FleischBerg Autos] Raising hook "..craneID.." for safe rotation @"..getRealTime().timestamp)
                moveHook(craneID, desiredZ, -1)
            elseif (barDirectionDeviation > 0.1) then
                -- Rotate the crane to its destination
				outputConsole("[FleischBerg Autos] Rotating crane "..craneID.." into position for dropoff @"..getRealTime().timestamp)
            ...
            elseif (craneID == 1 and vehicleDistanceFromBase < 70) then
				...
				outputConsole("[FleischBerg Autos] Moving hook 1 above drop zone @"..getRealTime().timestamp)
            ...
            elseif (craneID == 2 and math.abs(vehicleDistanceFromBase-65.5) > 1) then
				...
				outputConsole("[FleischBerg Autos] Moving hook 2 above marker @"..getRealTime().timestamp)
                moveHook(craneID,-1,65.5)
            elseif (craneID == 1 and AEROPLANES[vehicleModel] and rZ-14 > 0) then
                -- Lower big planes down safely so they don't get blown up when put down
				outputConsole("[FleischBerg Autos] Lowering hook 1 for secure plane drop @"..getRealTime().timestamp)
            ...
            elseif (craneID == 2 and TRAINS[vehicleModel] and rZ-13 > 0) then
				...
				outputConsole("[FleischBerg Autos] Lowering hook 2 for secure train drop @"..getRealTime().timestamp)
                moveHook(craneID, 12.8, -1)
            else
                -- Detach vehicle at destination
		        detachElements(vehicle)
                if (craneID == 2) then
					outputConsole("[FleischBerg Autos] Raising hook 2 after drop to marker @"..getRealTime().timestamp)
            ...
            if (hookDistance < 0.5 and getElementHealth(vehicle) >= 250) then
				...
				outputConsole("[FleischBerg Autos] Attaching vehicle to crane "..craneID.." @"..getRealTime().timestamp)
				attachElements(vehicle, CRANE_ROPE[craneID], 0, 0, -HOOK_BOAT_HEIGHT_OFFSET, vU-hU, vV-hV, vW-hW)
                if (craneID == 1) then
                    local craneTwoMoveIntoPositionSpeed = 0.5
                    if (vehicleModel == 608) then --tugstairs
                        craneTwoMoveIntoPositionSpeed = 0.25
                    end
					outputConsole("[FleischBerg Autos] Rotating crane 2 in anticipation of a handover from crane 1 @"..getRealTime().timestamp)
		            rotateCraneTo(2, 218, nil, craneTwoMoveIntoPositionSpeed) -- rotate crane 2 into position upon being grabbed by crane 1
                end
                return
            end
            ...
            if (craneAngleFromVehicle < 0.1 or craneAngleFromVehicle > 359.9) then
                ...
                local t = -1
                if (TRAINS[vehicleModel] or UNDRIVEABLE_TRAILERS[vehicleModel]) then
                    t = 3000
                end
				outputConsole("[FleischBerg Autos] Moving hook "..craneID.." to boat @"..getRealTime().timestamp)
				...
				outputConsole("[FleischBerg Autos] Rotating crane "..craneID.." above boat @"..getRealTime().timestamp)
    ...
    elseif (CRANE_STATE[craneID] == "available") then
        ...
        if (VEHICLE_CRANE_DATA[vehicleModel]) then
            ...
            if (r < 2) then
                if (craneID ~= 2) then
					...
					outputConsole("[FleischBerg Autos] Rotating crane 1 for fun @"..getRealTime().timestamp)
                    rotateCraneRelative(craneID, r)        
                else
                    r = math.random(36,376)
					outputConsole("[FleischBerg Autos] Rotating crane 2 for fun @"..getRealTime().timestamp)
...
function finalizeCraneRotation(_craneID)
	outputConsole("[FleischBerg Autos] Rotation of crane ".._craneID.." complete @"..getRealTime().timestamp)
    CRANE_ROTATING[_craneID] = false
    CRANE_TIMER[_craneID] = false
end
...
function finalizeCraneHookment(_craneID)
	outputConsole("[FleischBerg Autos] Hook movement of hook ".._craneID.." complete @"..getRealTime().timestamp)
    attachHook(_craneID)
    CRANE_HOOKING[_craneID] = false
    CRANE_TIMER[_craneID] = false
end
...
function craneDetectApproachingBoat(element, matchingDimension)
	...
	if (not vehicle or not VEHICLE_CRANE_DATA[vehicle]) then
		return
	end
	if (CRANE_STATE[1] == "available") then
		iprint("[FleischBerg Autos] Crane wasn't ready yet", 1)
		outputConsole("[FleischBerg Autos] Crane wasn't ready yet - 1 @"..getRealTime().timestamp)
	end
	if (CRANE_STATE[1] == "available" or CRANE_STATE[1] == "waiting for boat") then
		outputConsole("[FleischBerg Autos] Rotating crane 1 in anticipation of approaching boat @"..getRealTime().timestamp)
...
function abortCraneMovement(craneID)
    if (CRANE_ROTATING[craneID]) then
        iprint("[FleischBerg Autos] Crane was ill-prepared", craneID)
        outputConsole("[FleischBerg Autos] Crane was ill-prepared - ".. craneID.." @"..getRealTime().timestamp)
...
function craneGrab(craneID)
	if (CRANE_STATE[craneID] == "available") then
        iprint("[FleischBerg Autos] Crane was not aware this would happen ", craneID)
        outputConsole("[FleischBerg Autos] Crane was not aware this would happen - ".. craneID.." @"..getRealTime().timestamp)
...
function checkStoppedNearCrane()
	...
	if (VEHICLE_CRANE_DATA[getElementModel(vehicle)] and isElementWithinColShape(vehicle, REACH_CRANE2)) then
		craneGrab(2)
	elseif (VEHICLE_CRANE_DATA[getElementModel(vehicle)] and isElementWithinColShape(vehicle, REACH_CRANE1)) then
		craneGrab(1)
	end
end
addEventHandler("onClientPreRender", root, checkStoppedNearCrane)
