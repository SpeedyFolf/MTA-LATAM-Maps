...
function hideRamps()
	setElementPosition(AT_RAMPS[1], -51.1, -192.5, -32.0)
...
function showRamps()
	setElementPosition(AT_RAMPS[1], -51.1, -192.5, -2.0)
...
function erectAtHelperRamp(element, matchingDimension)
    ...
    if (vehicle == 577) then -- at400
        ...
        if (not AT_TUTORIAL_SHOWN) then
            ...
            g_midPlayBlurb = "Hey! Someone spilled a lot of junk next to the delivery point. That might come in handy!"
    ...
    elseif (vehicle ~= 577 and AT_TUTORIAL_SHOWN) then -- at400
        ...
        g_midPlayBlurb = "The large mess has unfortunately been cleaned up."
        g_showMidPlayTutorial = true
        setTimer(function()
            g_showMidPlayTutorial = false
        end, 7000, 1)
...
addEventHandler("onClientVehicleCollision", root,
    function(collider, damageImpulseMag, bodyPart, x, y, z, nx, ny, nz, force, model)
		...
		if (model ~= 13049 and model ~= 18256) then
			-- 13049: The piece of land just before the ramp
			-- 18256: Ramp (pier)
			return
		end
		DONT_DAMAGE = true
...
function handleVehicleDamage(attacker, weapon, loss, x, y, z, tire)
	if (not DONT_DAMAGE) then return end
	DONT_DAMAGE = false
	if (loss > 10) then 
		return 
	end
