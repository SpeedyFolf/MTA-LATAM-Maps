...
--outputChatBox("#E7D9B0You can get stats for the map by pressing #00FF00" ..helpText, 0, 0, 0, true)
...
function drawStats()
	...
	-- Draw vehicle's name
	dxDrawText("1% Winners", screenX*(st_offsets[1]+0.005), screenY*(st_offsets[2]-0.01+0.003), screenX, screenY, tocolor(0, 0, 0, statsAlpha), (screenY/1080)*4.02, (screenY/1080)*4.15, "beckett")
	dxDrawText("1% Winners", screenX*st_offsets[1], screenY*(st_offsets[2]-0.01), screenX*st_offsets[1], screenY, tocolor(175, 202, 230, statsAlpha), (screenY/1080)*4, (screenY/1080)*4, "beckett")
	
	local show = 20
	local box_height = screenY*s_offsets[4] * 0.9
	local textSize = box_height / 75 / ((show + 1) / 2)
	local offset = box_height / (show + 1)
...
addEventHandler("receiveStats", getRootElement(), function(recordsStats)		
	...
	for i, recordsData in pairs(recordsStats) do 
		...
		displayedRecords[i]["score"] = recordsData["score"]
		displayedRecords[i]["timestamp"] = recordsData["timestamp"]
	end
	
	if #recordsStats < 128 then
		for i = #recordsStats + 1, 128 do
			displayedRecords[i]["playername"] = "-- EMPTY --"
			displayedRecords[i]["score"] = 0
			displayedRecords[i]["timestamp"] = 0
		end
	end
