...
function resetCheckpoints(reason)
	if DEBUG then iprint("[Procedurally Generated Race]", "Reset Checkpoints", reason) end
							...
							if not getPedOccupiedVehicle(localPlayer) or (getPedOccupiedVehicle(localPlayer) and getElementModel(getPedOccupiedVehicle(localPlayer)) ~= vehicle["model"]) then
								...
								if vehicle["type"] ~= "Train" then
									...
									elseif not hit and waterHit then
										...
										iprint("[Procedurally Generated Race]", "FUCK FUCK FUCK")
...
function createCheckpoint(checkpoint, c_type)
	if DEBUG then iprint("[Procedurally Generated Race]", "cp created", checkpoint, c_type) end
...
addEventHandler("recieveMarkers", localPlayer, function(data)
	if getElementData(localPlayer, "gotdata") == 0 and data and #data > 0 then
		...
		-- Create pickups
		if race["pickups"] and #race["pickups"] > 0 then
		...
		-- Create Jumps
		if race["jumps"] and #race["jumps"] > 0 then
		...
		outputChatBox("#E7D9B0It's a#00FF00 " ..race["type"].. " #E7D9B0race with #00FF00" ..race["maxCP"].. " #E7D9B0checkpoints using #00FF00" ..vehicle["name"].. "" ..raceCustomText.. " #E7D9B0(#00FF00" ..race["raceDistance"].. " #E7D9B0m.)", 0, 0, 0, true)

		if not vehicle["trailer"] then outputChatBox("#E7D9B0You can get records for #00FF00" ..vehicle["name"].. " #E7D9B0by pressing #00FF00" ..helpText, 0, 0, 0, true)
