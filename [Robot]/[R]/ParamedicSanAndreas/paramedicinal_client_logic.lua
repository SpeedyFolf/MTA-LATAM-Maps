g_PatientStates = {} -- map of patient id -> picked_up
g_OpenSeats = g_MAX_PATIENTS_IN_VEHICLE
g_SpeedCheckState = nil -- we can only pick up once or drop off at a time

-- this was added later so g_OpenSeats could
-- technically be removed but I'm not bothering
local pickedUpPatients = {} -- list of patient ids
local lastHospitalDropOff = nil -- id of hospital
...
function initializeHospital(id)
	...
	addEventHandler("onClientColShapeHit", col, function(element)
		...
		triggerSpeedCheck(function()
			...
			triggerServerEvent(g_PATIENTS_DROPPED_OFF_EVENT, resourceRoot, g_MAX_PATIENTS_IN_VEHICLE - g_OpenSeats, getElementData(hospital, "id"))
			pickedUpPatients = {}
			lastHospitalDropOff = id
			g_OpenSeats = g_MAX_PATIENTS_IN_VEHICLE
...
function initializePatient(id)
	...
	addEventHandler("onClientColShapeHit", col, function(element)
		...
		triggerSpeedCheck(function()
			pickedUpPatients[#pickedUpPatients + 1] = id
...
function resetPickups()	
	...
	for _, id in ipairs(pickedUpPatients) do
		g_PatientStates[id] = true
...
addEventHandler("onClientPlayerSpawn", localPlayer, function()
	if lastHospitalDropOff then
		-- arbitrary timer because idk when the vehicle spawns
		setTimer(function()
			local hospital = g_HOSPITAL_POSITIONS[lastHospitalDropOff]
			setElementPosition(getPedOccupiedVehicle(localPlayer),
				getElementData(hospital, "posX"),
				getElementData(hospital, "posY"),
				-- arbitrary + 1 because idk how tall the ambulance is
				getElementData(hospital, "posZ") + 1)
		end, 1000, 1)
	end
end)