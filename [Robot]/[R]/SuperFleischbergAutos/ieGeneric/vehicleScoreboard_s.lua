RECORDS_DATABASE = dbConnect("sqlite", RECORDS_DATABASE_NAME)

addEventHandler("onPlayerDeliverVehicle", getRootElement(), function(oldTarget, time, vehicleModel, _, _)
	local isCheated = time < CHEATING_THRESHOLD_IN_MS

	if (RECORDS_DATABASE and not isCheated) then
		...
		for i, recordsData in pairs(recordsResults) do 
			if recordsData["playername"] == getPlayerName(source) and time == recordsData["score"] then
				...
				chatScoreNotificationText = chatScoreNotificationText .. VEHICLE_NAMES[vehicleModel]
