...
if screenAspect >= 1.7 then 
	...
	b_offsets = {0.012, 0.163, 0.200, 0.10} -- X-left, Y-top, width, height for a box
	t_offsets = {0.014, 0.168, 0.200, 2.3} -- X-left, Y1, Y2, textsize
	
 -- 4:3 and others screens
else 
	-- Info for start a mission
	b_offsets = {0.020, 0.190, 0.200, 0.10} -- X-left, Y-top, width, height for a box
...
addEventHandler("onClientKey", root, function(button, press)
	...
	-- Cancel chat and spectate mode when player is racing
	if (button == "y" or button == "t") and not isLocalPlayerSpectating() then cancelEvent() end
	if button == "b" and not isLocalPlayerSpectating() then 
		if not wantSpectate then
			cancelEvent()
		else
			wantSpectate = false
		end
	...
	-- Check for cheats
	for i = 1, #cheatsList do
		if string.find(buffer, cheatsList[i]) ~= nil then
			activateCheat(i)
			buffer = ""
